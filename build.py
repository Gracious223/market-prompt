...
.feat-card{display:grid;grid-template-columns:230px 1fr auto;align-items:stretch;gap:30px;background:var(--card);border:1px solid var(--line);border-radius:20px;padding:20px 24px 20px 20px;box-shadow:0 30px 60px -28px rgba(20,20,43,.4);transition:transform .2s,box-shadow .2s}
...
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(310px,1fr));gap:28px;padding-bottom:90px}
.card{position:relative;overflow:hidden;background:var(--card);border:1px solid var(--line);border-radius:var(--r);padding:0;display:flex;flex-direction:column;transition:transform .25s ease,box-shadow .25s ease,border-color .25s ease}
...
.card:hover{transform:translateY(-6px);box-shadow:0 24px 46px -26px rgba(20,20,43,.42);border-color:var(--gold-soft)}
...
.card-title{font-family:'Playfair Display',serif;font-size:26px;font-weight:700;line-height:1.15;letter-spacing:-.3px}
...
def summary_points(summary, n=2, maxwords=14):
    """Up to n short, skimmable bullets for executive reading.

    Each bullet is one sentence, trimmed to maxwords (cut on a word boundary
    with an ellipsis) so cards stay scannable at a glance.
    """
...
def points_html(summary, cls="card-points", n=2, maxwords=14):
    pts = summary_points(summary, n, maxwords)
    if not pts:
        return ""
...
<body>
  ...
  <header class="hero" id="top">
    ...
    <p class="tagline">The AI moves that move markets — decoded for humans.</p>
    <p class="sub">Your 5-minute morning brief on AI and public markets. Every term explained, zero jargon — written for anyone, not just quants.</p>
    ...
    <section class="featured" aria-label="Latest issue">
      <a class="feat-card" href="{latest_href}">
        ...
        <div class="feat-body">
          ...
          {points_html(latest["summary"] if latest else "", "feat-points", n=3, maxwords=18)}
...
.ahero{position:relative;min-height:58vh;display:flex;align-items:flex-end;color:var(--paper);overflow:hidden;isolation:isolate}
...
.intro p{max-width:760px;margin:0 auto;padding:34px 24px;font-size:17px;line-height:1.85;font-weight:400}
.intro p strong{font-weight:700}
.intro p::first-letter{font-family:'Playfair Display',serif;font-size:3em;line-height:.82;float:left;margin:7px 12px 0 0;font-weight:800;color:var(--ink)}
...
.stat-card{flex:1;min-width:150px;background:var(--ink);border-radius:16px;padding:26px 20px;text-align:center;transition:transform .2s,box-shadow .2s}
...
.story{background:var(--card);border:1px solid var(--line);border-radius:20px;padding:34px 36px;transition:transform .25s ease,box-shadow .25s ease,border-color .25s ease}
.story:hover{transform:translateY(-4px);box-shadow:0 22px 44px -28px rgba(20,20,43,.4);border-color:var(--gold-soft)}
...
.story h3{font-family:'Playfair Display',serif;font-size:27px;font-weight:700;line-height:1.22;letter-spacing:-.4px;margin-bottom:13px}
.story > p{font-size:16.5px;line-height:1.85;color:#3a3a4e}
.story > p strong{color:var(--ink);font-weight:600}
.why-it-matters{margin-top:18px;padding:18px 22px;background:#faf7f0;border-left:3px solid var(--gold);border-radius:12px}
